...
@savegroot.on(events.NewMessage(incoming=True, pattern='/unauth'))
...
@savegroot.on(events.NewMessage(incoming=True, pattern='/auth'))
...
@savegroot.on(events.NewMessage(incoming=True, pattern='/batch'))
    ...
    async with savegroot.conversation(event.chat_id) as conv: 
        if not s:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("Join Channel", url="http://t.me/savegroot")]])
    ...
    for i in range(len(ids)):
        ...
        try: 
            ...
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("Join Channel", url="https://t.me/savegroot")]])
            ...
            if int(fw.value) > 300:
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="http://t.me/savegroot")]])
        except Exception as e:
            #print(e)
            logger.info(e)
            await client.send_message(sender, f"An error occurred during cloning, batch will continue.\n\n**Error:** {str(e)}")
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/savegroot")]])
        n = i + 1
        if n == len(ids):
            return -2
...
TEXT = "👋 Hi, This is 'Paid Restricted Content Saver' bot Made with ❤️ by __**savegroot**__."

@savegroot.on(events.NewMessage(pattern=f"^{C}"))
async def start_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.inline("Cancel", data="cancel"),
         Button.inline("Cancel", data="cancel")],
        [Button.url("Join Channel", url="https://telegram.dog/savegroot")]
    ]
